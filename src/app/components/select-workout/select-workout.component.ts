...
export class SelectWorkoutComponent implements OnInit {
    ...
    this.newWorkout = {
      ...
      save: () => {
        if (!this.newWorkout.name) {
          this.toaster.error('Please enter a workout name');
          return;
        }
        if (!this.newWorkout.exercises.length) {
          this.toaster.error('Please add at least one exercise to the workout');
          return;
        }

        this.gs.saveProgram({name: this.newWorkout.name, exercises: this.newWorkout.exercises});
      }
    }
  ...
  addExercise = (exercise: any) => {
    this.newWorkout.exercises.push(exercise);
  }
  removeExercise = (exercise: any) => {
    const index = this.newWorkout.exercises.indexOf(exercise);
    if (index > -1) {
